...
def extract_features1(Y_flat, encoder="PCA", pca_variance_ratio=0.999, seed=1024, vae_config=None):
    print(f"[INFO] Raw Y_flat min={np.min(Y_flat)}, max={np.max(Y_flat)}, mean={np.mean(Y_flat)}, std={np.std(Y_flat)}")
    ...
    if encoder == "PCA":
        print("[INFO] Using PCA for feature extraction.")
        pca_model = PCA(n_components=pca_variance_ratio)
        Y_encoded = pca_model.fit_transform(Y_flat)

        print(f"[PCA] n_components_: {pca_model.n_components_}")
        print(f"[PCA] Sum explained variance: {np.sum(pca_model.explained_variance_ratio_)}")
        ...
        learning_rate = vae_config.get("learning_rate", 1e-4)
        batch_size = vae_config.get("batch_size", 64)
        kl_weight = vae_config.get("kl_weight", 1.0)  # 预留β-VAE的kl权重
        ...
        for epoch in range(epochs):
            ...
            for step, x_batch in enumerate(dataset):
                with tf.GradientTape() as tape:
                    x_decoded, mean, logvar = vae_model(x_batch)
                    loss = compute_vae_loss(x_batch, x_decoded, mean, logvar) * kl_weight
        ...
        # ✅ 关键改动：正确处理encoder输出
        # mean_logvar = vae_model.encoder.predict(Y_flat)
        # mean, logvar = tf.split(mean_logvar, num_or_size_splits=2, axis=1)
        # latent = mean + tf.random.normal(tf.shape(mean)) * tf.exp(0.5 * logvar)  # ✅ reparameterization
        # Y_encoded = latent.numpy()
        mean_logvar = vae_model.encoder(Y_flat)
        mean, logvar = tf.split(mean_logvar, num_or_size_splits=2, axis=1)
    ...
    return Y_encoded, pca_model, mean_val, std_val
...
# old code that blend training with test when PCA
def run_training1(train_dict, model_type="GPR", kernel="RBF_White", pca_variance_ratio=0.999, encoder="PCA",vae_config=None, seed=42, return_pred=True):
    """
    ...
    # 2. 特征提取
    Y_pca, feature_extractor,mean_val,std_val = extract_features(Y_flat, encoder=encoder, pca_variance_ratio=pca_variance_ratio,seed=seed, vae_config=vae_config)
...
def multi_seed_run_and_select_best(train_dict, emulator_name, model_type="GPR", kernel="RBF_White",
                                   encoder="VAE", vae_config=None, output_dir="outputs",seeds=None):
    """
    ...
    # === 保存seeds vs R²曲线图 ===
    save_seed_vs_score_plot(
        df=df,
        emulator_name=emulator_name,
        output_dir=output_dir,
        seed=best_seed,
        latent_dim=vae_config.get("latent_dim"),
        epochs=vae_config.get("epochs"),
        learning_rate=vae_config.get("learning_rate"),
        batch_size=vae_config.get("batch_size"),
